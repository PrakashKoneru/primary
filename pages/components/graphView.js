...
const GraphView = ({ selectedNav, ...props }) => {
	...
	useEffect(() => {
		const baseURL = process.env.NODE_ENV === 'production' ? `/primary/be/primaryLenders/loans/${selectedNav.split(" ")[0]}` : `http://localhost:5003/primaryLenders/loans/${selectedNav.split(" ")[0]}`;
		axios.get(baseURL, {
			headers: {
